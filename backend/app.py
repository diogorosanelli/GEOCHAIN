...
def register_event():
    ...
    geo_data = get_geographic_data(oid)
    ...
    tx_receipt = register_event_on_blockchain(oid, event_type, geo_hash, details)
    response = {
...
@app.route('/api/event/list/<int:oid>', methods=['GET'])
def get_events(oid):
    """
    Endpoint para obter os eventos registrados para um determinado lote.
    """
    events = get_events_from_blockchain(oid)
